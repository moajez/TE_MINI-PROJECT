...
export const Summary: React.FC<SummaryProps> = ({ course, user, onLogin }) => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      const response = await fetch("http://localhost:5000/api/syllabus/courses", {
        headers: {
  ...
  useEffect(() => {
    const fetchSyllabusTopics = async () => {
      ...
      const response = await fetch(`http://localhost:5000/api/syllabus/syllabus/${selectedCourse.courseCode}`, {
        headers: {
          "Authorization": `Bearer ${userToken}`
        }
      });
