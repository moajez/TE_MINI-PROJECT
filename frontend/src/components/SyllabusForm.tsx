...
const SyllabusForm = () => {
  ...
  const [syllabus, setSyllabus] = useState([{ id: "1.1", topic: "", number: "1.1", _id: uuidv4() }]);
  const [moduleCount, setModuleCount] = useState(1);

  const addTopic = () => {
    const lastTopic = syllabus.filter((s) => s.id.startsWith(`${moduleCount}.`)).length;
    const newId = `${moduleCount}.${lastTopic + 1}`;
    setSyllabus([...syllabus, { id: newId, topic: "", number: newId, _id: uuidv4() }]);
  };

  const addModule = () => {
    setModuleCount(moduleCount + 1);
    setSyllabus([...syllabus, { id: `${moduleCount + 1}.1`, topic: "", number: `${moduleCount + 1}.1`, _id: uuidv4() }]);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-gray-800 rounded-lg shadow-xl">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300">Syllabus Topics</label>
